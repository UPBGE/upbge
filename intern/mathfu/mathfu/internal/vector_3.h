...
class Vector<T, 3> {
 public:
  ...
  inline const T (&Data() const)[3] WARN_UNUSED_RESULT {
    return data_;
  }
  ...
  inline Vector<T, 3> Normalized() const WARN_UNUSED_RESULT { return NormalizedHelper(*this); }

  inline Vector<T, 3> SafeNormalized(const Vector<T, 3>& v) const WARN_UNUSED_RESULT {
  ...
  static inline WARN_UNUSED_RESULT Vector<T, 3> FromType(const CompatibleT& compatible) {
    return FromTypeHelper<T, d, CompatibleT>(compatible);
  }

  template <typename CompatibleT>
  static inline WARN_UNUSED_RESULT CompatibleT ToType(const Vector<T, 3>& v) {
    return ToTypeHelper<T, d, CompatibleT>(v);
  }

  static inline WARN_UNUSED_RESULT T DotProduct(const Vector<T, 3>& v1, const Vector<T, 3>& v2) {
    return DotProductHelper(v1, v2);
  }

  static inline WARN_UNUSED_RESULT Vector<T, 3> HadamardProduct(const Vector<T, 3>& v1,
                                             const Vector<T, 3>& v2) {
  ...
  static inline WARN_UNUSED_RESULT Vector<T, 3> Lerp(const Vector<T, 3>& v1,
                                  const Vector<T, 3>& v2, const T percent) {
    return LerpHelper(v1, v2, percent);
  }

  static inline WARN_UNUSED_RESULT Vector<T, 3> RandomInRange(const Vector<T, 3>& min,
                                           const Vector<T, 3>& max) {
    return RandomInRangeHelper(min, max);
  }

  static inline WARN_UNUSED_RESULT Vector<T, 3> Max(const Vector<T, 3>& v1,
                                 const Vector<T, 3>& v2) {
    return MaxHelper(v1, v2);
  }

  static inline WARN_UNUSED_RESULT Vector<T, 3> Min(const Vector<T, 3>& v1,
                                 const Vector<T, 3>& v2) {
