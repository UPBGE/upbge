...
static bool gwl_window_state_set(GWL_Window *win, const GHOST_TWindowState state)
{
  ...
  return gwl_window_state_set_for_xdg(win->xdg_decor->toplevel, state, state_current);
}
...
static void gwl_window_activate(GWL_Window *win)
{
  ...
  if (activation_manager == nullptr) [[unlikely]] {
    return;
  }
...
static void gwl_window_pending_actions_handle(GWL_Window *win)
{
  ...
  if (actions[PENDING_WINDOW_CURSOR_SHAPE_REFRESH]) {
    /* Best-effort refresh: failure is acceptable when pending (e.g. cursor theme unavailable). */
    (void)gwl_window_cursor_shape_refresh(
...
static void gwl_window_frame_update_from_pending_no_lock(GWL_Window *win)
{
  ...
  /* Run last to account for the updated window.
   * Note that we could attempt to minimize when this runs however as `layout_callback`
   * is expected to be low overhead (since it runs on redraw), there isn't any advantage,
   * further, failure to run is likely to result in strange bugs (window & decoration mismatch).
   *
   * For reference re-running is needed on:
   * - DPI change.
   * - Frame size change.
   * - Frame state change.
   *
   * But not needed on activate/deactivate. */
#ifdef WITH_GHOST_CSD
  if (GWL_WindowCSD *xdg_csd = win->xdg_csd) {
...
static int outputs_max_scale_or_default(const std::span<GWL_Output *const> outputs,
...
static int outputs_uniform_scale_or_default(const std::span<GWL_Output *const> outputs,
                                            const int32_t scale_default,
                                            int *r_scale_fractional)
{
      ...
      preferred_device_(preferred_device)
{
   ...
   * follows the method used in SDL 3.16. */
  {
    ...
    if (system_->xdg_decor_manager_get()) {
       ...
       * or to use client side decorations. */
      const uint32_t mode = (use_window_frame && system_->use_window_frame_csd_get() == false) ?
...
const std::span<GWL_Output *const> GHOST_WindowWayland::outputs_get() const
{
...
{
  return std::erase(window_->outputs, output) > 0;
}
...
xdg_toplevel *GHOST_WindowWayland::xdg_toplevel_get()
{
  if (window_->xdg_decor) {
    return window_->xdg_decor->toplevel;
  }
  return nullptr;
}

GHOST_TWindowState GHOST_WindowWayland::xdg_toplevel_state_get()
{
  /* This mainly exists for the moment for CSD buttons.
   * #getState can't be used as it causes a deadlock. */
  return gwl_window_state_get(window_);
}

GHOST_TCSD_Type GHOST_WindowWayland::csd_elem_active_type_get() const
{
  GHOST_ASSERT(this->system_->use_window_frame_csd_get(), "caller must ensure");
  return window_->xdg_csd->active_type;
}
void GHOST_WindowWayland::csd_elem_active_type_set(GHOST_TCSD_Type type)
{
  GHOST_ASSERT(this->system_->use_window_frame_csd_get(), "caller must ensure");
  window_->xdg_csd->active_type = type;
}
const GHOST_CSD_Elem *GHOST_WindowWayland::csd_layout(int *r_num)
{
  GHOST_ASSERT(this->system_->use_window_frame_csd_get(), "caller must ensure");
