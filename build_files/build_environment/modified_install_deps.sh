...
getopt \
-o s:i:t:h \
--long source:,install:,tmp:,info:,threads:,help,show-deps,no-sudo,no-confirm,static,with-all,with-opencollada,\
  ...
  if [ ! -d $_inst ]; then
    ...
    if $STATIC; then
        ./configure --disable-shared \
            --prefix=$_inst --libdir=$_inst/lib --enable-ipv6 \
...
compile_Boost() {
  ...
  if [ ! -d $_inst ]; then
    ...
    if $STATIC; then
        ./b2 link=static -j$THREADS -a $BOOST_BUILD_MODULES \
...
compile_OCIO() {
  ...
  if [ ! -d $_inst ]; then
    ...
    # Static by default cmake_d="$cmake_d -D OCIO_BUILD_STATIC=ON"
