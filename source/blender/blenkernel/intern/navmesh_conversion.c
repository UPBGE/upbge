                                  ...
                                  const int *dtrisToPolysMap)
{
  ...
  unsigned short *newPoly = MEM_callocN(sizeof(unsigned short) * capacity,
  ...
  for (polyidx = 0; polyidx < npolys; polyidx++) {
    ...
    unsigned char *traversedTris = MEM_callocN(sizeof(unsigned char) * dtrisNum,
                                               "buildPolygonsByDetailedMeshes traversedTris");
    ...
    while (tri != btri || edge != bedge) {
      ...
      if (neighbortri == 0xffff || dtrisToPolysMap[neighbortri] != polyidx + 1) {
        if (nv == capacity) {
          ...
          newPolyBig = MEM_callocN(sizeof(unsigned short) * capacity,
    ...
    adjustedPoly = MEM_callocN(sizeof(unsigned short) * nv,
...

{
  ...
  vertsPerPoly = 6;
  polys = MEM_callocN(sizeof(unsigned short) * npolys * vertsPerPoly * 2,
