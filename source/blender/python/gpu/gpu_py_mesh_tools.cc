...
PyDoc_STRVAR(
    ...
    "   :arg obj: Evaluated :class:`bpy.types.Object` owning the mesh\n"
    "      (use ``obj.evaluated_get(depsgraph)``).\n"
    "   :type obj: :class:`bpy.types.Object`\n"
    "   :arg ssbo_positions: :class:`gpu.types.GPUStorageBuf` containing ``vec4`` per\n"
    "      vertex (object-space positions).\n"
    "   :type ssbo_positions: :class:`gpu.types.GPUStorageBuf`\n"
...
PyDoc_STRVAR(pygpu_mesh_compute_free_doc,
             ...
             "   This also resets internal flags like `mesh.is_using_gpu_deform`.\n"
              ...
              "\n"
              "   :arg obj: Mesh owner object, evaluated or original.\n"
              "   :type obj: :class:`bpy.types.Object`\n"
              "   :return: ``None``.\n"
              "   :rtype: None\n"
              "\n");
...
PyDoc_STRVAR(
    ...
    ".. function:: run_compute_mesh(obj, shader, bindings, config=None, dispatch_count=0)\n"
    "\n"
    "   Run a custom compute shader on a mesh.\n"
    "\n"
    "   :arg obj: Evaluated mesh object with a ready batch cache.\n"
    "   :type obj: :class:`bpy.types.Object`\n"
    "   :arg shader: GLSL compute shader source code.\n"
    "   :type shader: str\n"
    "   :arg bindings: Sequence of 5-tuples ``(binding_index, buffer, qualifier, type_name, bind_name)``.\n"
    "   :type bindings: Sequence[tuple]\n"
    "   :arg config: Optional callable returning a Python dict for specialization constants\n"
    "      and push constants.\n"
    "   :type config: Callable | None\n"
    "   :arg dispatch_count: Number of invocations. If ``0``, defaults to mesh vertex count.\n"
    ...
    "\n"
    "   - Top-level entries with scalar values (``int``, ``float``, ``bool``) are treated as\n"
    "     specialization constants and declared at shader creation time.\n"
    "   - A special key ``'push_constants'`` may contain a dict mapping uniform names to values.\n"
    "   - Values can be ``float``/``int``/``bool`` or a sequence of floats/ints for arrays;\n"
    "     they are applied immediately before dispatch via ``GPU_shader_uniform_*``.\n"
    "\n"
    ...
    "\n"
    "   :return: Integer status: ``0`` = Success, ``1`` = NotReady (deferred), ``2`` = Error.\n"
    "   :rtype: int\n"
    "\n");
