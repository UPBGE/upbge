...
static PlayBackRefuseInfo compute_gpu_playback_decision(Object &ob, Mesh &mesh)
{
  ...
  for (ModifierData *md = static_cast<ModifierData *>(ob_orig->modifiers.first); md; md = md->next)
  {
    ...
    /* Check if this is a deform modifier */
    const ModifierTypeInfo *mti = BKE_modifier_get_info((ModifierType)md->type);
    if (!mti || mti->type != ModifierTypeType::OnlyDeform) {
      continue; /* Not a deform modifier */
    }
    ...
    switch (md->type) {
      ...
      case eModifierType_Warp: {
        ...
        requests_gpu = wmd && (wmd->deform_method & WAR_DEFORM_METHOD_GPU);
        explicitly_cpu = wmd && !(wmd->deform_method & WAR_DEFORM_METHOD_GPU);
        break;
      }
      ...
      case eModifierType_MeshDeform: {
        ...
        requests_gpu = mmd && (mmd->deform_method & MESHDEFORM_DEFORM_METHOD_GPU);
        explicitly_cpu = mmd && !(mmd->deform_method & MESHDEFORM_DEFORM_METHOD_GPU);
        break;
      }
      default:
        break;
