...
void EEVEE_effects_init(EEVEE_Data *vedata)
{
	...
	IDProperty *props = BKE_scene_layer_engine_evaluated_get(scene_layer, COLLECTION_MODE_NONE, RE_engine_id_BLENDER_GAME);
	...
	if (BKE_collection_engine_property_value_get_bool(props, "bloom_enable")) {
		...
		DRW_framebuffer_init(&fbl->bloom_blit_fb, &draw_engine_game_type,
		...
		for (int i = 0; i < effects->bloom_iteration_ct; ++i) {
			...
			DRW_framebuffer_init(&fbl->bloom_down_fb[i], &draw_engine_game_type,
		...
		for (int i = 0; i < effects->bloom_iteration_ct - 1; ++i) {
			...
			DRW_framebuffer_init(&fbl->bloom_accum_fb[i], &draw_engine_game_type,
			                    (int)texsize[0], (int)texsize[1],
			                    &tex_bloom, 1);
		}
	...
	if (BKE_collection_engine_property_value_get_bool(props, "dof_enable")) {
		...
		if (rv3d->persp == RV3D_CAMOB && v3d->camera) {
			...
			DRW_framebuffer_init(&fbl->dof_down_fb, &draw_engine_game_type, buffer_size[0], buffer_size[1], tex_down, 3);

			DRWFboTexture tex_scatter_far = {&txl->dof_far_blur, DRW_TEX_RGBA_16, DRW_TEX_FILTER};
			DRW_framebuffer_init(&fbl->dof_scatter_far_fb, &draw_engine_game_type, buffer_size[0], buffer_size[1], &tex_scatter_far, 1);

			DRWFboTexture tex_scatter_near = {&txl->dof_near_blur, DRW_TEX_RGBA_16, DRW_TEX_FILTER};
			DRW_framebuffer_init(&fbl->dof_scatter_near_fb, &draw_engine_game_type, buffer_size[0], buffer_size[1], &tex_scatter_near, 1);
	...
	if (effects->enabled_effects != 0) {
		...
		DRW_framebuffer_init(&fbl->effect_fb, &draw_engine_game_type,
	...
	DRW_framebuffer_init(&fbl->minmaxz_fb, &draw_engine_game_type,
