...
static void planar_pool_ensure_alloc(EEVEE_Data *vedata, int num_planar_ref)
{
	...
	if (num_planar_ref > 0) {
		...
		DRW_framebuffer_init(&fbl->planarref_fb, &draw_engine_game_type,
...
void EEVEE_lightprobes_init(EEVEE_SceneLayerData *sldata, EEVEE_Data *UNUSED(vedata))
{
	...
	DRW_framebuffer_init(&sldata->probe_fb, &draw_engine_game_type, PROBE_RT_SIZE, PROBE_RT_SIZE, &tex_probe, 1);
	...
	// DRW_framebuffer_init(&vedata->fbl->minmaxz_fb, &draw_engine_game_type, PROBE_RT_SIZE / 2, PROBE_RT_SIZE / 2, &tex_minmaxz, 1);
...
void EEVEE_lightprobes_cache_finish(EEVEE_SceneLayerData *sldata, EEVEE_Data *vedata)
{
	...
	DRW_framebuffer_init(&sldata->probe_filter_fb, &draw_engine_game_type, PROBE_OCTAHEDRON_SIZE, PROBE_OCTAHEDRON_SIZE, &tex_filter, 1);
