...
void EEVEE_lights_cache_finish(EEVEE_SceneLayerData *sldata)
{
	...
	DRW_framebuffer_init(&sldata->shadow_cube_target_fb, &draw_engine_game_type, 512, 512, tex_cube_target, 2);

	DRWFboTexture tex_cube = {&sldata->shadow_depth_cube_pool, DRW_TEX_R_16, DRW_TEX_FILTER};
	DRW_framebuffer_init(&sldata->shadow_cube_fb, &draw_engine_game_type, 512, 512, &tex_cube, 1);

	DRWFboTexture tex_cascade = {&sldata->shadow_depth_cascade_pool, DRW_TEX_DEPTH_24, DRW_TEX_FILTER | DRW_TEX_COMPARE};
	DRW_framebuffer_init(&sldata->shadow_cascade_fb, &draw_engine_game_type, 512, 512, &tex_cascade, 1);
