...
static struct GPUTexture *create_ggx_lut_texture(int UNUSED(w), int UNUSED(h))
{
	...
	DRW_framebuffer_init(&fb, &draw_engine_game_type, w, h, &tex_filter, 1);
...
struct GPUMaterial *EEVEE_material_world_lightprobe_get(struct Scene *scene, World *wo)
{
	return GPU_material_from_nodetree(
	    scene, wo->nodetree, &wo->gpumaterial, &DRW_engine_viewport_game_type,
	    VAR_WORLD_PROBE,
        ...
        bool use_ao, bool use_bent_normals)
{
	...
	mat = GPU_material_from_nodetree(
	    scene, ma->nodetree, &ma->gpumaterial, &DRW_engine_viewport_game_type,
	    options,
