...
gpu::Texture *get_noise_hash_texture(Mesh *mesh_owner,
                                     Object *deformed_eval,
                                     const std::string &key)
{
  gpu::Texture *tex = bke::BKE_mesh_gpu_internal_texture_get(mesh_owner, key);
  if (!tex) {
    /* permutation table: upload first 256 entries as floats (0..255). */
    static float packed_hashf[256];
    for (int i = 0; i < 256; ++i) {
      packed_hashf[i] = float(BLI_noise_hash_uchar_512[i]);
    }

    tex = GPU_texture_create_1d("noise_hash_texture",
                                256,
                                1,
                                gpu::TextureFormat::SFLOAT_32, /* single-component float */
...
gpu::Texture *get_noise_hashvect_texture(Mesh *mesh_owner,
                                         Object *deformed_eval,
                                         const std::string &key)
{
  gpu::Texture *tex = bke::BKE_mesh_gpu_internal_texture_get(mesh_owner, key);
  if (!tex) {
    /* pack flat vec3 array (hashvectf[256*3]) into RGBA32F (256 texels of vec4) */
    static float packed_hashvectf[256 * 4];
    for (int i = 0; i < 256; ++i) {
      int s = i * 3;
      int d = i * 4;
      packed_hashvectf[d + 0] = hashvectf[s + 0];
      packed_hashvectf[d + 1] = hashvectf[s + 1];
      packed_hashvectf[d + 2] = hashvectf[s + 2];
      packed_hashvectf[d + 3] = 0.0f;
    }

    tex = GPU_texture_create_1d("noise_hashvect_texture",
                                256,
                                1,
                                gpu::TextureFormat::SFLOAT_32_32_32_32,
                                GPU_TEXTURE_USAGE_SHADER_READ,
                                nullptr);
    GPU_texture_update(tex, GPU_DATA_FLOAT, packed_hashvectf);
    bke::BKE_mesh_gpu_internal_texture_ensure(mesh_owner, deformed_eval, key, tex);
  }
  return tex;
}

gpu::Texture *get_noise_hashpnt_texture(Mesh *mesh_owner,
                                        Object *deformed_eval,
                                        const std::string &key)
{
  gpu::Texture *tex = bke::BKE_mesh_gpu_internal_texture_get(mesh_owner, key);
  if (!tex) {
    /* pack flat vec3 array (hashpntf[256*3]) into RGBA32F (256 texels of vec4) */
    static float packed_hashpntf[256 * 4];
    for (int i = 0; i < 256; ++i) {
      int s = i * 3;
      int d = i * 4;
      packed_hashpntf[d + 0] = hashpntf[s + 0];
      packed_hashpntf[d + 1] = hashpntf[s + 1];
      packed_hashpntf[d + 2] = hashpntf[s + 2];
      packed_hashpntf[d + 3] = 0.0f;
    }

    tex = GPU_texture_create_1d("noise_hashpnt_texture",
                                256,
                                1,
                                gpu::TextureFormat::SFLOAT_32_32_32_32,
                                GPU_TEXTURE_USAGE_SHADER_READ,
                                nullptr);
    GPU_texture_update(tex, GPU_DATA_FLOAT, packed_hashpntf);
    bke::BKE_mesh_gpu_internal_texture_ensure(mesh_owner, deformed_eval, key, tex);
  }
  return tex;
}
...
/* Return permutation value (0..255). Masked to 255 to preserve CPU uchar behavior. */
uint gpu_hash(int idx)
{
  int i = idx & 255;
  float f = texelFetch(u_hash_buf, i, 0).r;
  /* f already 0..255 as uploaded from C++ */
  return uint(f) & 255u;
}

/* hashvectf was a flat float[768] (256 * vec3). The shader used indices like 3*X.
 * We store one vec3 per texel; pass the original flat index (multiple-of-3).
...
/* Fetch an entry from hashpntf3 (vec3[256] originally) */
vec3 gpu_hashpntf3_fetch(int idx)
{
  /* idx already in 0..255 */
  int i = idx & 255;
  return texelFetch(u_hashpntf3_buf, i, 0).xyz;
}

/* HASHPNT macro replacement used by legacy Voronoi helpers */
vec3 gpu_hashpntf_legacy_buf(int x, int y, int z)
{
  uint zu = uint(z) & 255u;
  uint yu = uint(y) & 255u;
  uint xu = uint(x) & 255u;

  uint h0 = gpu_hash(int(zu));
  uint h1 = gpu_hash(int((h0 + yu) & 255u));
  uint h2 = gpu_hash(int((h1 + xu) & 255u));
  return gpu_hashpntf3_fetch(int(h2));
}

/* Replacement of noise3_perlin that uses the texel buffers above.
 * You can drop this in place of the array-based version. */
float noise3_perlin(vec3 vec)
{
  ...
  int i = int(gpu_hash(bx0));
  int j = int(gpu_hash(bx1));

  int b00 = int(gpu_hash(i + by0));
  int b10 = int(gpu_hash(j + by0));
  int b01 = int(gpu_hash(i + by1));
  int b11 = int(gpu_hash(j + by1));
  ...
  idx = 3 * (b00 + bz0);
  h = gpu_hashvectf_by_hidx(idx);
  u = rx0 * h.x + ry0 * h.y + rz0 * h.z;

  idx = 3 * (b10 + bz0);
  h = gpu_hashvectf_by_hidx(idx);
  v = rx1 * h.x + ry0 * h.y + rz0 * h.z;
  float a = mix(u, v, sx);

  idx = 3 * (b01 + bz0);
  h = gpu_hashvectf_by_hidx(idx);
  u = rx0 * h.x + ry1 * h.y + rz0 * h.z;
  idx = 3 * (b11 + bz0);
  h = gpu_hashvectf_by_hidx(idx);
  v = rx1 * h.x + ry1 * h.y + rz0 * h.z;
  float bval = mix(u, v, sx);

  float c = mix(a, bval, sy);

  idx = 3 * (b00 + bz1);
  h = gpu_hashvectf_by_hidx(idx);
  u = rx0 * h.x + ry0 * h.y + rz1 * h.z;
  idx = 3 * (b10 + bz1);
  h = gpu_hashvectf_by_hidx(idx);
  v = rx1 * h.x + ry0 * h.y + rz1 * h.z;
  a = mix(u, v, sx);

  idx = 3 * (b01 + bz1);
  h = gpu_hashvectf_by_hidx(idx);
  u = rx0 * h.x + ry1 * h.y + rz1 * h.z;
  idx = 3 * (b11 + bz1);
  h = gpu_hashvectf_by_hidx(idx);
  v = rx1 * h.x + ry1 * h.y + rz1 * h.z;
  bval = mix(u, v, sx);

  float d = mix(a, bval, sy);
...
float orgBlenderNoise(float x, float y, float z)
{
   ...
   * Use gpu_hash helper that reads permutation from a texel buffer. */
  int ixm = ix & 255;
  int iym = iy & 255;
  int ix1m = (ix + 1) & 255;
  int iym1 = (iy + 1) & 255;

  int hix = int(gpu_hash(ixm));
  int hix1 = int(gpu_hash(ix1m));

  b00 = int(gpu_hash((hix + iym) & 255));
  b10 = int(gpu_hash((hix1 + iym) & 255));
  b01 = int(gpu_hash((hix + iym1) & 255));
  b11 = int(gpu_hash((hix1 + iym1) & 255));

  b20 = iz & 255;
  b21 = (iz + 1) & 255;

  /* Sample 8 corners of the cube using gpu_hashvectf_by_hidx to fetch gradients. */
  int h_idx;
  vec3 h;

  /* corner 0 */
  i_tmp = cn1 * cn2 * cn3;
  h_idx = 3 * int(gpu_hash((b20 + b00) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * ox + h.y * oy + h.z * oz);

  /* corner 1 */
  i_tmp = cn1 * cn2 * cn6;
  h_idx = 3 * int(gpu_hash((b21 + b00) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * ox + h.y * oy + h.z * jz);

  /* corner 2 */
  i_tmp = cn1 * cn5 * cn3;
  h_idx = 3 * int(gpu_hash((b20 + b01) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * ox + h.y * jy + h.z * oz);

  /* corner 3 */
  i_tmp = cn1 * cn5 * cn6;
  h_idx = 3 * int(gpu_hash((b21 + b01) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * ox + h.y * jy + h.z * jz);

  /* corner 4 */
  i_tmp = cn4 * cn2 * cn3;
  h_idx = 3 * int(gpu_hash((b20 + b10) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * jx + h.y * oy + h.z * oz);

  /* corner 5 */
  i_tmp = cn4 * cn2 * cn6;
  h_idx = 3 * int(gpu_hash((b21 + b10) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * jx + h.y * oy + h.z * jz);

  /* corner 6 */
  i_tmp = cn4 * cn5 * cn3;
  h_idx = 3 * int(gpu_hash((b20 + b11) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
  n += i_tmp * (h.x * jx + h.y * jy + h.z * oz);

  /* corner 7 */
  i_tmp = cn4 * cn5 * cn6;
  h_idx = 3 * int(gpu_hash((b21 + b11) & 255));
  h = gpu_hashvectf_by_hidx(h_idx);
...
float newPerlinU(float x, float y, float z)
{
  ...
  int A = int(gpu_hash(X)) + Y;
  int AA = int(gpu_hash(A & 255)) + Z;
  int AB = int(gpu_hash((A + 1) & 255)) + Z;
  int B = int(gpu_hash((X + 1) & 255)) + Y;
  int BA = int(gpu_hash(B & 255)) + Z;
  int BB = int(gpu_hash((B + 1) & 255)) + Z;

  float g1 = newperlin_grad(int(gpu_hash(AA & 255)), x, y, z);
  float g2 = newperlin_grad(int(gpu_hash(BA & 255)), x - 1.0, y, z);
  float a1 = mix(g1, g2, u);

  g1 = newperlin_grad(int(gpu_hash(AB & 255)), x, y - 1.0, z);
  g2 = newperlin_grad(int(gpu_hash(BB & 255)), x - 1.0, y - 1.0, z);
  float b1 = mix(g1, g2, u);
  float c1 = mix(a1, b1, v);

  g1 = newperlin_grad(int(gpu_hash((AA + 1) & 255)), x, y, z - 1.0);
  g2 = newperlin_grad(int(gpu_hash((BA + 1) & 255)), x - 1.0, y, z - 1.0);
  a1 = mix(g1, g2, u);

  g1 = newperlin_grad(int(gpu_hash((AB + 1) & 255)), x, y - 1.0, z - 1.0);
  g2 = newperlin_grad(int(gpu_hash((BB + 1) & 255)), x - 1.0, y - 1.0, z - 1.0);
  b1 = mix(g1, g2, u);
