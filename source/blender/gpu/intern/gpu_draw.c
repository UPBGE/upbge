	...
	if (use_matcap) {
		...
		if (ob->totcol == 0) {
			...
			if (glsl) {
				...
				GPU_material_from_blender(GMS.gscene, &defmaterial, GMS.is_opensubdiv, false, false);
			}
		...
		for (a = 1; a <= ob->totcol; a++) {
			...
			gpumat = glsl ? GPU_material_from_blender(GMS.gscene, ma, GMS.is_opensubdiv, false, false) : NULL;
...
int GPU_object_material_bind(int nr, void *attribs)
{
	...
	if (GMS.gboundmat) {
		...
		GPU_material_unbind(GPU_material_from_blender(GMS.gscene, GMS.gboundmat, GMS.is_opensubdiv, false, false));
		GMS.gboundmat = NULL;
	}
	...
	if (GMS.lastretval) {
		...
		if (gattribs && GMS.gmatbuf[nr]) {
			...
			GPUMaterial *gpumat = GPU_material_from_blender(GMS.gscene, mat, GMS.is_opensubdiv, false, false);
