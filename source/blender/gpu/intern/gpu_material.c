...
static void do_material_tex(GPUShadeInput *shi)
{
	...
	for (tex_nr = 0; tex_nr < MAX_MTEX; tex_nr++) {
		...
		if (ma->mtex[tex_nr]) {
			mtex = ma->mtex[tex_nr];
			use_dynuni = (mtex->texflag & MTEX_DYNAMIC_UNIFORM);
			tex = mtex->tex;
			...
			if (mtex->mapto & MAP_PARALLAX) {
				...
				GPU_link(mat, "parallax_out", texco,
					...
					use_dynuni ? 
						GPU_dynamic_uniform(&mtex->parallaxsteps, GPU_DYNAMIC_TEX_PARALLAXSTEP, NULL) :
						GPU_uniform(&mtex->parallaxsteps),
					use_dynuni ?
						GPU_dynamic_uniform(&mtex->parallaxbumpsc, GPU_DYNAMIC_TEX_PARALLAXBUMP, NULL) :
						GPU_uniform(&mtex->parallaxbumpsc),
					GPU_uniform(&discard),
					&parco);
			}
			else {
		...
		if (ma->mtex[tex_nr]) {
			...
			if (mtex->mapto & (MAP_COL | MAP_COLSPEC | MAP_COLMIR)) {
				...
				if (mtex->mapto & MAP_COL) {
					...
					if (mtex->colfac == 1.0f && !use_dynuni) colfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->colfac, GPU_DYNAMIC_TEX_COLFAC, NULL) :
							GPU_uniform(&mtex->colfac), stencil, &colfac);
					}
				...
				if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && (mtex->mapto & MAP_COLSPEC)) {
					...
					if (mtex->colspecfac == 1.0f && !use_dynuni) colspecfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->colspecfac, GPU_DYNAMIC_TEX_SPECFAC, NULL) :
							GPU_uniform(&mtex->colspecfac), stencil, &colspecfac);
					}
				...
				if (mtex->mapto & MAP_COLMIR) {
					...
					if (mtex->mirrfac == 1.0f && !use_dynuni) colmirfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->mirrfac, GPU_DYNAMIC_TEX_MIRROR, NULL) :
							GPU_uniform(&mtex->mirrfac), stencil, &colmirfac);
					}
			...
			if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && (mtex->mapto & MAP_NORM)) {
				if (tex->type == TEX_IMAGE) {
					...
					if (tex->imaflag & TEX_NORMALMAP) {
						...
						if (norfac == 1.0f && !GPU_link_changed(stencil) && !use_dynuni) {
							shi->vn = newnor;
						}
						else {
							tnorfac = use_dynuni ? 
								GPU_dynamic_uniform(&mtex->norfac, GPU_DYNAMIC_TEX_NORMAL, NULL) :
								GPU_uniform(&mtex->norfac);
			...
			if ((mtex->mapto & MAP_VARS)) {
				...
				if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && mtex->mapto & MAP_REF) {
					...
					if (mtex->difffac == 1.0f && !use_dynuni) difffac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->difffac, GPU_DYNAMIC_TEX_COLINTENS, NULL) :
							GPU_uniform(&mtex->difffac), stencil, &difffac);
					}
				...
				if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && mtex->mapto & MAP_SPEC) {
					...
					if (mtex->specfac == 1.0f && !use_dynuni) specfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->specfac, GPU_DYNAMIC_TEX_SPECINTENS, NULL) :
							GPU_uniform(&mtex->specfac), stencil, &specfac);
					}
				...
				if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && mtex->mapto & MAP_EMIT) {
					...
					if (mtex->emitfac == 1.0f && !use_dynuni) emitfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->emitfac, GPU_DYNAMIC_TEX_EMIT, NULL) :
							GPU_uniform(&mtex->emitfac), stencil, &emitfac);
					}
				...
				if (!(mat->scene->gm.flag & GAME_GLSL_NO_EXTRA_TEX) && mtex->mapto & MAP_HAR) {
					...
					if (mtex->hardfac == 1.0f && !use_dynuni) hardfac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->hardfac, GPU_DYNAMIC_TEX_HARDNESS, NULL) :
							GPU_uniform(&mtex->hardfac), stencil, &hardfac);
					}
				...
				if (mtex->mapto & MAP_ALPHA) {
					...
					if (mtex->alphafac == 1.0f && !use_dynuni) alphafac = stencil;
					else {
						GPU_link(mat, "math_multiply", use_dynuni ? 
							GPU_dynamic_uniform(&mtex->alphafac, GPU_DYNAMIC_TEX_ALPHA, NULL) :
							GPU_uniform(&mtex->alphafac), stencil, &alphafac);
					}
