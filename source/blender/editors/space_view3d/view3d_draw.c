...
static void backdrawview3d(Scene *scene, wmWindow *win, ARegion *ar, View3D *v3d)
{
	...
	if (win->multisamples != USER_MULTISAMPLE_NONE) {
		...
		if (!rv3d->gpuoffscreen) {
			rv3d->gpuoffscreen = GPU_offscreen_create(w, h, 0, GPU_OFFSCREEN_DEPTH_COMPARE, error);
        ...
        GPUFX *fx, GPUOffScreen *ofs, char err_out[256])
{
	...
	if (own_ofs) {
		/* bind */
		ofs = GPU_offscreen_create(sizex, sizey, full_samples ? 0 : samples, GPU_OFFSCREEN_DEPTH_COMPARE, err_out);
		if (ofs == NULL) {
			return NULL;
		}
	}
