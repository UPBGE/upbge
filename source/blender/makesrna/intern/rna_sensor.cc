...
static void rna_def_sensor(BlenderRNA *brna)
{
  ...
  srna = RNA_def_struct(brna, "Sensor", nullptr);
  ...
  RNA_def_property_string_funcs(prop, nullptr, nullptr, "rna_Sensor_name_set");
  ...
  RNA_def_property_enum_funcs(prop, nullptr, "rna_Sensor_type_set", "rna_Sensor_type_itemf");
  RNA_def_property_ui_text(prop, "Type", "");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "pin", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_PIN);
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", SENS_DEACTIVATE);
  RNA_def_property_ui_text(prop, "Active", "Set active state of the sensor");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "show_expanded", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_SHOW);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "level", 1);
  RNA_def_property_ui_text(prop,
                           "Level",
                           "Level detector, trigger controllers of new states "
                           "(only applicable upon logic state transition)");
  RNA_def_property_boolean_funcs(prop, nullptr, "rna_Sensor_level_set");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_pulse_true_level", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "pulse", SENS_PULSE_REPEAT);
  RNA_def_property_ui_text(
      prop, "Pulse True Level", "Activate TRUE level triggering (pulse mode)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_pulse_false_level", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "pulse", SENS_NEG_PULSE_MODE);
  RNA_def_property_ui_text(
      prop, "Pulse False Level", "Activate FALSE level triggering (pulse mode)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "tick_skip", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "freq");
  RNA_def_property_ui_text(
      prop,
      "Skip",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "tap", 1);
  RNA_def_property_boolean_funcs(prop, nullptr, "rna_Sensor_tap_set");
  RNA_def_property_ui_text(
      prop, "Tap", "Trigger controllers only for an instant, even while the sensor remains true");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_ui_text(
      prop, "Controllers", "The list containing the controllers connected to the sensor");
  RNA_def_property_collection_funcs(prop,
                                    "rna_Sensor_controllers_begin",
                                    "rna_iterator_array_next",
                                    "rna_iterator_array_end",
                                    "rna_iterator_array_dereference_get",
                                    "rna_Sensor_controllers_length",
                                    nullptr,
                                    nullptr,
                                    nullptr);
...
static void rna_def_near_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      prop, "Property", "Only look for objects with this property (blank = all objects)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "resetdist");
  RNA_def_property_ui_text(
      prop, "Reset Distance", "The distance where the sensor forgets the actor");
  RNA_def_property_range(prop, 0.0f, 10000.0f);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_mouse_sensor(BlenderRNA *brna)
{
  ...
  static const EnumPropertyItem mouse_event_items[] = {
      ...
      {BL_SENS_MOUSE_BUTTON4, "BUTTON4CLICK", 0, "Button 4", ""},
      {BL_SENS_MOUSE_BUTTON5, "BUTTON5CLICK", 0, "Button 5", ""},
  ...
  static const EnumPropertyItem prop_mouse_type_items[] = {
      {SENS_COLLISION_PROPERTY,
       ...
       ICON_ACTION,
       "Property",
       "Use a property for ray intersections"},  // CHOOSE BETTER ICON
  ...
  RNA_def_property_ui_text(
      prop, "Mouse Event", "Type of event this mouse sensor should trigger on");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_pulse", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_MOUSE_FOCUS_PULSE);
  RNA_def_property_ui_text(
      prop, "Pulse", "Moving the mouse over a different object generates a pulse");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_string_sdna(prop, nullptr, "matname");
  RNA_def_property_ui_text(
      prop, "Material", "Only look for objects with this material (blank = all objects)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_x_ray", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_RAY_XRAY);
  RNA_def_property_ui_text(
      prop, "X-Ray", "Toggle X-Ray option (see through objects that don't have the property)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
static void rna_def_keyboard_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "key");
  RNA_def_property_enum_items(prop, rna_enum_event_type_items);
  RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_UI_EVENTS);
  RNA_def_property_enum_funcs(prop, nullptr, "rna_Sensor_keyboard_key_set", nullptr);
  RNA_def_property_ui_text(prop, "Key", "");
  RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_ID_WINDOWMANAGER);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "modifier_key_1", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "qual");
  RNA_def_property_enum_items(prop, rna_enum_event_type_items);
  RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_UI_EVENTS);
  RNA_def_property_enum_funcs(prop, nullptr, "rna_Sensor_keyboard_modifier_set", nullptr);
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "qual2");
  RNA_def_property_enum_items(prop, rna_enum_event_type_items);
  RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_UI_EVENTS);
  RNA_def_property_enum_funcs(prop, nullptr, "rna_Sensor_keyboard_modifier2_set", nullptr);
  ...
  RNA_def_property_string_sdna(prop, nullptr, "targetName");
  RNA_def_property_ui_text(
      prop, "Target", "Property that receives the keystrokes in case a string is logged");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "log", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "toggleName");
  RNA_def_property_ui_text(
      prop, "Log Toggle", "Property that indicates whether to log keystrokes as a string");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_all_keys", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "type", 1);
...
static void rna_def_property_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_struct_ui_text(
      srna, "Property Sensor", "Sensor to detect values and changes in values of properties");
  ...
  RNA_def_property_string_sdna(prop, nullptr, "maxvalue");
...
static void rna_def_armature_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_struct_ui_text(
      srna, "Armature Sensor", "Sensor to detect values and changes in values of IK solver");
  ...
  RNA_def_property_ui_text(
      prop, "Constraint Name", "Identify the bone constraint to check value from");
  RNA_def_property_update(prop, NC_LOGIC, "rna_Sensor_Armature_update");
...
static void rna_def_actuator_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_struct_ui_text(
      srna, "Actuator Sensor", "Sensor to detect state modifications of actuators");
  RNA_def_struct_sdna_from(srna, "bActuatorSensor", "data");

  /* XXX if eventually have Logics using RNA 100%, we could use the actuator data-block isntead of
   * its name */
  prop = RNA_def_property(srna, "actuator", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "name");
  RNA_def_property_ui_text(
      prop, "Actuator", "Actuator name, actuator active state modifications will be detected");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_delay_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_DELAY_REPEAT);
  RNA_def_property_ui_text(
      prop,
      "Repeat",
...
static void rna_def_collision_sensor(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_pulse", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "mode", SENS_COLLISION_PULSE);
  RNA_def_property_ui_text(
      prop, "Pulse", "Change to the set of colliding objects generates pulse");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_material", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "mode", SENS_COLLISION_MATERIAL);
  RNA_def_property_ui_text(prop, "M/P", "Toggle collision on material or property");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  /*XXX to make a setFunction to create a lookup with all materials in Blend File (not only this
   * object mat.) */
  prop = RNA_def_property(srna, "material", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "materialName");
  RNA_def_property_ui_text(
      prop, "Material", "Only look for objects with this material (blank = all objects)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
#  if 0
	...
	RNA_def_property_pointer_sdna(prop, nullptr, "ma");
...
static void rna_def_radar_sensor(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "distance", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "range");
...
static void rna_def_random_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      prop, "Seed", "Initial seed of the generator (choose 0 for not random)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_ray_sensor(BlenderRNA *brna)
{
  ...
  static const EnumPropertyItem prop_ray_type_items[] = {{SENS_COLLISION_PROPERTY,
                                                          "PROPERTY",
                                                          ICON_ACTION,
                                                          "Property",
...
#  endif
  ...
  prop = RNA_def_property(srna, "use_x_ray", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "mode", SENS_RAY_XRAY);
  RNA_def_property_ui_text(
      prop,
      "X-Ray Mode",
  ...
  prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "axisflag");
  RNA_def_property_enum_items(prop, axis_items);
...
static void rna_def_movement_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "localflag", SENS_MOVEMENT_LOCAL);
...
static void rna_def_joystick_sensor(BlenderRNA *brna)
{
  ...
  RNA_def_property_translation_context(prop, BLT_I18NCONTEXT_UI_EVENTS);
  RNA_def_property_ui_text(
      prop, "Event Type", "The type of event this joystick sensor is triggered on");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_all_events", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SENS_JOY_ANY_EVENT);
  RNA_def_property_ui_text(
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "axisf");
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "axis_single");
  ...
  RNA_def_property_ui_text(
      prop, "Stick Axis", "Which stick single axis (vertical/horizontal/other) to detect");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
