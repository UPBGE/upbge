...
static float rna_ConstraintActuator_limitmin_get(struct PointerRNA *ptr)
{
  ...
  if (ca->flag & ACT_CONST_LOCX)
    fp = ca->minloc;
  else if (ca->flag & ACT_CONST_LOCY)
    fp = ca->minloc + 1;
  else if (ca->flag & ACT_CONST_LOCZ)
    fp = ca->minloc + 2;
  else if (ca->flag & ACT_CONST_ROTX)
    fp = ca->minrot;
  else if (ca->flag & ACT_CONST_ROTY)
    fp = ca->minrot + 1;
  else
    fp = ca->minrot + 2;
...
static float rna_ConstraintActuator_distance_get(struct PointerRNA *ptr)
{
  ...
  if (ca->mode & (ACT_CONST_DIRPX | ACT_CONST_DIRNX))
    fp = ca->minloc;
  else if (ca->mode & (ACT_CONST_DIRPY | ACT_CONST_DIRNY))
    fp = ca->minloc + 1;
  else
    fp = ca->minloc + 2;
...
static void rna_Actuator_constraint_detect_material_set(struct PointerRNA *ptr, bool value)
{
  ...
  bool old_value = (ca->flag & ACT_CONST_MATERIAL) ? true : false;
...
{
  bActuator *act = (bActuator *)ptr->data;
  bStateActuator *sa = (bStateActuator *)act->data;
...
/* Always keep in alphabetical order */
const EnumPropertyItem *rna_Actuator_type_itemf(bContext *C,
                                                PointerRNA *ptr,
                                                PropertyRNA */*prop*/,
                                                bool *r_free)
{
...
static void rna_Actuator_Armature_update(Main */*bmain*/,
                                         Scene */*scene*/,
                                         PointerRNA *ptr)
{
  bActuator *act = (bActuator *)ptr->data;
...
static void rna_SteeringActuator_navmesh_set(PointerRNA *ptr,
                                             PointerRNA value,
...
static void rna_def_actuator(BlenderRNA *brna)
{
  ...
  srna = RNA_def_struct(brna, "Actuator", nullptr);
  ...
  RNA_def_property_string_funcs(prop, nullptr, nullptr, "rna_Actuator_name_set");
  RNA_def_struct_name_property(srna, prop);

  prop = RNA_def_property(srna, "type", PROP_ENUM, PROP_NONE);
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_enum_items(prop, actuator_type_items);
  RNA_def_property_enum_funcs(prop, nullptr, "rna_Actuator_type_set", "rna_Actuator_type_itemf");
  RNA_def_property_ui_text(prop, "Type", "");

  prop = RNA_def_property(srna, "pin", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_PIN);
  RNA_def_property_ui_text(
      prop, "Pinned", "Display when not linked to a visible states controller");
  RNA_def_property_ui_icon(prop, ICON_UNPINNED, 1);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "show_expanded", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_SHOW);
  RNA_def_property_ui_text(prop, "Expanded", "Set actuator expanded in the user interface");
  RNA_def_property_ui_icon(prop, ICON_TRIA_RIGHT, 1);

  prop = RNA_def_property(srna, "active", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_DEACTIVATE);
...
#  endif
  ...
  static const EnumPropertyItem prop_blend_items[] = {{ACT_ACTION_BLEND, "BLEND", 0, "Blend", ""},
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "end_reset", 1);
  RNA_def_property_ui_text(
      prop,
      "Continue",
  ...
  RNA_def_property_float_sdna(prop, nullptr, "sta");
  ...
  RNA_def_property_int_sdna(prop, nullptr, "blendin");
  ...
  RNA_def_property_range(
      prop, 0, 32766); /* This should match BL_ActionManager::MAX_ACTION_LAYERS - 1 */
  ...
  RNA_def_property_ui_text(
      prop, "Layer Weight", "How much of the previous layer to blend into this one");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "frame_property", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "frameProp");
  RNA_def_property_ui_text(
      prop, "Frame Property", "Assign the action's current frame number to this property");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  /* booleans */
  prop = RNA_def_property(srna, "use_additive", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_IPOADD);
  RNA_def_property_boolean_funcs(prop, nullptr, "rna_ActionActuator_add_set");
  RNA_def_property_ui_text(
      prop,
      "Add",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_IPOFORCE);
  RNA_def_property_boolean_funcs(prop, nullptr, "rna_ActionActuator_force_set");
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_IPOLOCAL);
  RNA_def_property_ui_text(
      prop, "L", "Let the Action act in local coordinates, used in Force and Add mode");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "apply_to_children", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_IPOCHILD);
...
#  ifdef __NLA_ACTION_BY_MOTION_ACTUATOR
  ...
  RNA_def_property_float_sdna(prop, nullptr, "stridelength");
...
static void rna_def_object_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_pointer_sdna(prop, nullptr, "reference");
  ...
  RNA_def_property_ui_text(
      prop, "Damping Frames", "Number of frames to reach the target velocity");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "proportional_coefficient", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "forcerot[0]");
  RNA_def_property_ui_range(prop, 0.0, 200.0, 10, 2);
  RNA_def_property_ui_text(
      prop, "Proportional Coefficient", "Typical value is 60x integral coefficient");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "integral_coefficient", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "forcerot[1]");
  RNA_def_property_ui_range(prop, 0.0, 3.0, 10, 2);
  RNA_def_property_float_funcs(prop, nullptr, "rna_ObjectActuator_integralcoefficient_set", nullptr);
  RNA_def_property_ui_text(
      prop,
  ...
  RNA_def_property_float_sdna(prop, nullptr, "forcerot[2]");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "dloc[0]");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "forceloc");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_range(prop, -10000.0, 10000.0, 10, 2);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "forcerot");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_range(prop, -10000.0, 10000.0, 10, 2);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "linearvelocity");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_range(prop, -10000.0, 10000.0, 10, 2);
  RNA_def_property_ui_text(
      prop,
  ...
  RNA_def_property_float_sdna(prop, nullptr, "angularvelocity");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_range(prop, -10000.0, 10000.0, 10, 2);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_DLOC_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_DROT_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_FORCE_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_TORQUE_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_LIN_VEL_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_ANG_VEL_LOCAL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_ADD_LIN_VEL);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_ADD_CHAR_LOC);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_SERVO_LIMIT_X);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_SERVO_LIMIT_Y);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_SERVO_LIMIT_Z);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CHAR_JUMP);
  RNA_def_property_ui_text(
      prop, "Jump", "Make the character jump using the settings in the physics properties");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_camera_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      prop, "Damping", "Strength of the constraint that drives the camera behind the target");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
static void rna_def_sound_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.min_gain");
  RNA_def_property_ui_range(prop, 0.0, 1.0, 1, 2);
  RNA_def_property_ui_text(
      prop, "Minimum Gain", "The minimum gain of the sound, no matter how far it is away");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "gain_3d_max", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.max_gain");
  RNA_def_property_ui_range(prop, 0.0, 1.0, 1, 2);
  RNA_def_property_ui_text(
      prop, "Maximum Gain", "The maximum gain of the sound, no matter how near it is");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "distance_3d_reference", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.reference_distance");
  RNA_def_property_ui_range(prop, 0.0, FLT_MAX, 1, 2);
  RNA_def_property_ui_text(
      prop, "Reference Distance", "The distance where the sound has a gain of 1.0");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "distance_3d_max", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.max_distance");
  RNA_def_property_ui_range(prop, 0.0, FLT_MAX, 1, 2);
  RNA_def_property_ui_text(
      prop, "Maximum Distance", "The maximum distance at which you can hear the sound");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "rolloff_factor_3d", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.rolloff_factor");
  RNA_def_property_ui_range(prop, 0.0, 5.0, 1, 2);
  RNA_def_property_ui_text(
      prop, "Rolloff", "The influence factor on volume depending on distance");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "cone_outer_gain_3d", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.cone_outer_gain");
  RNA_def_property_ui_range(prop, 0.0, 1.0, 1, 2);
  RNA_def_property_ui_text(
      prop,
      "Cone Outer Gain",
  ...
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.cone_outer_angle");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "sound3D.cone_inner_angle");
  RNA_def_property_ui_range(prop, 0.0, DEG2RADF(360.0f), 1, 2);
...
static void rna_def_property_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      prop, "Value", "The name of the property or the value to use (use \"\" around strings)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
static void rna_def_constraint_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "type");
  RNA_def_property_enum_items(prop, prop_type_items);
  RNA_def_property_enum_funcs(prop, nullptr, "rna_ConstraintActuator_type_set", nullptr);
  ...
  RNA_def_property_float_funcs(
      prop, "rna_ConstraintActuator_limitmin_get", "rna_ConstraintActuator_limitmin_set", nullptr);
  ...
  RNA_def_property_float_funcs(
      prop, "rna_ConstraintActuator_limitmax_get", "rna_ConstraintActuator_limitmax_set", nullptr);
  RNA_def_property_ui_range(prop, -2000.f, 2000.f, 1, 2);
  RNA_def_property_ui_text(prop, "Max", "");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "damping", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "damp");
  RNA_def_property_ui_range(prop, 0, 100, 1, 1);
  RNA_def_property_ui_text(
      prop, "Damping", "Damping factor: time constant (in frame) of low pass filter");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  /* ACT_CONST_TYPE_DIST */
  prop = RNA_def_property(srna, "range", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_funcs(
      prop, "rna_ConstraintActuator_range_get", "rna_ConstraintActuator_range_set", nullptr);
  ...
  RNA_def_property_float_funcs(
      prop, "rna_ConstraintActuator_distance_get", "rna_ConstraintActuator_distance_set", nullptr);
  RNA_def_property_ui_range(prop, -2000.f, 2000.f, 1, 2);
  ...
  RNA_def_property_string_sdna(prop, nullptr, "matprop");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  ...
  RNA_def_property_int_sdna(prop, nullptr, "rotdamp");
  RNA_def_property_ui_range(prop, 0, 100, 1, 1);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "maxrot");
  RNA_def_property_array(prop, 3);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "minloc[0]");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "maxloc[0]");
  RNA_def_property_range(prop, 0.0f, DEG2RADF(180.0f));
  RNA_def_property_ui_text(
      prop,
      "Max Angle",
  ...
  RNA_def_property_float_funcs(
      prop, "rna_ConstraintActuator_spring_get", "rna_ConstraintActuator_spring_set", nullptr);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "maxrot[0]");
  RNA_def_property_ui_range(prop, 0.0, 1.0, 10, 2);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_DISTANCE);
  RNA_def_property_ui_text(
      prop, "Force Distance", "Force distance of object to point of impact of ray");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_local", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_LOCAL);
  RNA_def_property_ui_text(prop, "L", "Set ray along object's axis or global axis");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_normal", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_NORMAL);
  RNA_def_property_ui_text(
      prop,
      "N",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_PERMANENT);
  RNA_def_property_ui_text(
      prop, "PER", "Persistent actuator: stays active even if ray does not reach target");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  /*XXX to use an enum instead of a flag if possible */
  prop = RNA_def_property(srna, "use_material_detect", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_MATERIAL);
  RNA_def_property_ui_text(prop, "M/P", "Detect material instead of property");
  RNA_def_property_boolean_funcs(prop, nullptr, "rna_Actuator_constraint_detect_material_set");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_fh_paralel_axis", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_CONST_DOROTFH);
...
static void rna_def_edit_object_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "dyn_operation");
  ...
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Object", "Add this Object and all its children (can't be on a visible layer)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_pointer_sdna(prop, nullptr, "me");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop,
      "Mesh",
  ...
  RNA_def_property_pointer_funcs(prop, nullptr, "rna_Actuator_editobject_mesh_set", nullptr, nullptr);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_EDOB_ADD_OBJECT_DUPLI);
  RNA_def_property_ui_text(
  ...
  RNA_def_property_boolean_sdna(
      prop, nullptr, "dyn_operation_flag", ACT_EDOB_SUSPEND_PHY_CHILDREN_RECURSIVE);
  ...
  RNA_def_property_boolean_sdna(
      prop, nullptr, "dyn_operation_flag", ACT_EDOB_SUSPEND_PHY_FREE_CONSTRAINTS);
  ...
  RNA_def_property_boolean_sdna(
      prop, nullptr, "dyn_operation_flag", ACT_EDOB_RESTORE_PHY_CHILDREN_RECURSIVE);
  RNA_def_property_ui_text(prop, "Restore Children Dynamics", "Apply to all children");
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_local_linear_velocity", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "localflag", ACT_EDOB_LOCAL_LINV);
  RNA_def_property_ui_text(prop, "L", "Apply the transformation locally");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_local_angular_velocity", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "localflag", ACT_EDOB_LOCAL_ANGV);
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_EDOB_REPLACE_MESH_NOGFX);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_EDOB_REPLACE_MESH_PHYS);
  RNA_def_property_ui_text(
      prop,
      "Phys",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_TRACK_3D);
...
static void rna_def_scene_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Camera Object", "Set this Camera (leave empty to refer to self object)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
static void rna_def_collection_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_COLLECTION_SUSPEND_LOGIC);
  RNA_def_property_ui_text(prop, "Logic", "Suspend/Resume Logic");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "use_physics", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_COLLECTION_SUSPEND_PHYSICS);
...
static void rna_def_random_actuator(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "property", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "propname");
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "int_arg_1", 1);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "float_arg_1");
  RNA_def_property_range(prop, 0.0, 1.0);
  RNA_def_property_ui_text(
      prop, "Chance", "Pick a number between 0 and 1, success if it's below this value");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  /* ACT_RANDOM_INT_CONST */
  prop = RNA_def_property(srna, "int_value", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "int_arg_1");
  ...
  RNA_def_property_ui_text(
      prop, "Min", "Choose a number from a range: lower boundary of the range");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "int_max", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "int_arg_2");
  RNA_def_property_range(prop, -1000, 1000);
  RNA_def_property_ui_text(
      prop, "Max", "Choose a number from a range: upper boundary of the range");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "float_arg_2");
  RNA_def_property_range(prop, -1000.0, 1000.0);
  RNA_def_property_ui_text(
      prop, "Max", "Choose a number from a range: upper boundary of the range");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  ...
  RNA_def_property_range(prop, -1000.0, 1000.0);
  RNA_def_property_ui_text(
      prop, "SD", "A normal distribution: standard deviation of the distribution");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
...
static void rna_def_message_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_string_sdna(prop, nullptr, "toPropName");
  RNA_def_property_ui_text(
      prop, "To", "Optional, send message to objects with this name only, or empty to broadcast");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "subject", PROP_STRING, PROP_NONE);
  RNA_def_property_ui_text(
      prop, "Subject", "Optional, message subject (this is what can be filtered on)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "body_type", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "bodyType");
  ...
  RNA_def_property_string_sdna(prop, nullptr, "body");
  ...
  RNA_def_property_string_sdna(prop, nullptr, "body");
  RNA_def_property_ui_text(
      prop, "Prop Name", "The message body will be set by the Property Value");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_game_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
  /*XXX to do: an operator that calls file_browse with relative_path on and blender filtering
   * active */
}
...
static void rna_def_vibration_actuator(BlenderRNA *brna)
{
  ...
  static EnumPropertyItem prop_mode_items[] = {{ACT_VIBRATION_PLAY, "PLAY", 0, "Play", ""},
  ...
  RNA_def_property_int_sdna(prop, nullptr, "joyindex");
  ...
  RNA_def_property_int_sdna(prop, nullptr, "duration");
  ...
  RNA_def_property_range(prop, 0.0, 1.0);
  RNA_def_property_ui_text(
      prop, "Strength Low Freq", "Joystick vibration strength for low frequency motor");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "joy_strength_right", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "strength_right");
  RNA_def_property_range(prop, 0.0, 1.0);
  RNA_def_property_ui_text(
      prop, "Strength High Freq", "Joystick vibration strength for high frequency motor");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
...
static void rna_def_visibility_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_VISIBILITY_INVISIBLE);
  RNA_def_property_ui_text(
      prop,
      "Visible",
...
static void rna_def_twodfilter_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_pointer_sdna(prop, nullptr, "text");
  ...
  RNA_def_property_int_sdna(prop, nullptr, "int_arg");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "float_arg");
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", 1);
...
static void rna_def_parent_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_PARENT_COMPOUND);
  RNA_def_property_ui_text(prop,
                           "Compound",
  ...
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "flag", ACT_PARENT_GHOST);
...
static void rna_def_armature_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_string_sdna(prop, nullptr, "posechannel");
  ...
  RNA_def_property_string_sdna(prop, nullptr, "constraint");
  ...
  RNA_def_property_pointer_sdna(prop, nullptr, "subtarget");
  RNA_def_property_struct_type(prop, "Object");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop,
                           "Secondary Target",
...
static void rna_def_steering_actuator(BlenderRNA *brna)
{
  ...
  static const EnumPropertyItem facingaxis_items[] = {{1, "X", 0, "X", ""},
                                                      {2, "Y", 0, "Y", ""},
                                                      {3, "Z", 0, "Z", ""},
                                                      {4, "NEG_X", 0, "-X", ""},
                                                      {5, "NEG_Y", 0, "-Y", ""},
  ...
  RNA_def_property_float_sdna(prop, nullptr, "velocity");
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_STEERING_SELFTERMINATED);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_STEERING_ENABLEVISUALIZATION);
  ...
  RNA_def_property_int_sdna(prop, nullptr, "updateTime");
  ...
  RNA_def_property_struct_type(prop, "Object");
  RNA_def_property_pointer_sdna(prop, nullptr, "navmesh");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Navigation Mesh Object", "Navigation mesh");
  RNA_def_property_pointer_funcs(prop, nullptr, "rna_SteeringActuator_navmesh_set", nullptr, nullptr);
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "facing", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_STEERING_AUTOMATICFACING);
  RNA_def_property_ui_text(prop, "Facing", "Enable automatic facing");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "facing_axis", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "facingaxis");
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_STEERING_NORMALUP);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_STEERING_LOCKZVEL);
  RNA_def_property_ui_text(
...
static void rna_def_mouse_actuator(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_VISIBLE);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_USE_AXIS_X);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_USE_AXIS_Y);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_RESET_X);
  RNA_def_property_ui_text(
      prop,
      "Reset",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_RESET_Y);
  RNA_def_property_ui_text(
      prop,
      "Reset",
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_LOCAL_X);
  RNA_def_property_ui_text(prop, "Local", "Apply rotation locally");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "local_y", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", ACT_MOUSE_LOCAL_Y);
  RNA_def_property_ui_text(prop, "Local", "Apply rotation locally");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "threshold_x", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "threshold[0]");
  RNA_def_property_ui_range(prop, 0, 0.5, 1, 3);
  RNA_def_property_ui_text(
      prop, "Threshold", "Amount of X motion before mouse movement will register");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "threshold_y", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "threshold[1]");
  RNA_def_property_ui_range(prop, 0, 0.5, 1, 3);
  RNA_def_property_ui_text(
      prop, "Threshold", "Amount of Y motion before mouse movement will register");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "object_axis_x", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "object_axis[0]");
  RNA_def_property_enum_items(prop, prop_object_axis_items);
  RNA_def_property_ui_text(
      prop, "Object Axis", "Local object axis mouse movement in the X direction will apply to");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "object_axis_y", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "object_axis[1]");
  RNA_def_property_enum_items(prop, prop_object_axis_items);
  RNA_def_property_ui_text(
      prop, "Object Axis", "Local object axis mouse movement in the Y direction will apply to");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "sensitivity_x", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "sensitivity[0]");
  ...
  RNA_def_property_float_sdna(prop, nullptr, "sensitivity[1]");
  RNA_def_property_ui_range(prop, -100.0, 100.0, 0.2, 3);
  ...
  RNA_def_property_float_sdna(prop, nullptr, "limit_x[0]");
  RNA_def_property_ui_range(prop, DEG2RADF(-3600.0f), 0.0, 9, 3);
  RNA_def_property_ui_text(
      prop, "Min", "Maximum negative rotation allowed by X mouse movement (0 for infinite)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "max_x", PROP_FLOAT, PROP_ANGLE);
  RNA_def_property_float_sdna(prop, nullptr, "limit_x[1]");
  RNA_def_property_ui_range(prop, 0.0, DEG2RADF(3600.0f), 9, 3);
  RNA_def_property_ui_text(
      prop, "Max", "Maximum positive rotation allowed by X mouse movement (0 for infinite)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "min_y", PROP_FLOAT, PROP_ANGLE);
  RNA_def_property_float_sdna(prop, nullptr, "limit_y[0]");
  RNA_def_property_ui_range(prop, DEG2RADF(-3600.0f), 0.0, 9, 3);
  RNA_def_property_ui_text(
      prop, "Min", "Maximum negative rotation allowed by Y mouse movement (0 for infinite)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);

  prop = RNA_def_property(srna, "max_y", PROP_FLOAT, PROP_ANGLE);
  RNA_def_property_float_sdna(prop, nullptr, "limit_y[1]");
  RNA_def_property_ui_range(prop, 0.0, DEG2RADF(3600.0f), 9, 3);
  RNA_def_property_ui_text(
      prop, "Max", "Maximum positive rotation allowed by Y mouse movement (0 for infinite)");
  RNA_def_property_update(prop, NC_LOGIC, nullptr);
}
