...
static void rna_def_light_shadow(StructRNA *srna, bool sun)
{
  ...
  prop = RNA_def_property(srna, "use_nvidia_shadows", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "mode", LA_SHADOWS_NVIDIA);
  RNA_def_property_ui_text(prop, "Use NVIDIA shadows", "Use NVIDIA shadows");
  RNA_def_property_update(prop, 0, "rna_Light_update");
