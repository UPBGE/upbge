...
void BL_ArmatureObject::LoadChannels()
{
  if (m_poseChannels->GetCount() == 0) {
    for (bPoseChannel *pchan = (bPoseChannel *)m_pose->chanbase.first; pchan;
...
void BL_ArmatureObject::ProcessReplica()
{
  ...
  m_controlledConstraints = static_cast<CListValue<BL_ArmatureConstraint> *>(
...
PyObject *BL_ArmatureObject::pyattr_get_constraints(PyObjectPlus *self_v,
...
PyObject *BL_ArmatureObject::pyattr_get_channels(PyObjectPlus *self_v,
                                                 const KX_PYATTRIBUTE_DEF *attrdef)
{
  BL_ArmatureObject *self = static_cast<BL_ArmatureObject *>(self_v);
...
KX_PYMETHODDEF_DOC_NOARGS(BL_ArmatureObject, draw, "Draw Debug Armature")
{
