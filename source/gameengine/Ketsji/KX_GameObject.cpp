...
RAS_MeshUser *KX_GameObject::GetCurrentMeshUser() const
{
	return m_currentMeshUser;
}

RAS_Deformer *KX_GameObject::GetDeformer() const
{
...
void KX_GameObject::AddDefaultMeshUser()
{
	for (size_t i = 0; i < m_meshes.size(); ++i) {
		...
		m_defaultMeshUser.reset(m_meshes[i]->AddMeshUser(&m_clientInfo, deformer));
...
void KX_GameObject::UpdateBuckets()
{
	...
	m_currentMeshUser->SetPassIndex(m_passIndex);
...
{
	return (m_currentMeshUser != nullptr);
}
...
void KX_GameObject::UpdateLod(KX_Scene *scene, const mt::vec3& cam_pos, float lodfactor)
{
	...
	RAS_MeshUser *meshUser = m_lodUser.GetMesh(this, m_clientInfo, scene, distance2);
