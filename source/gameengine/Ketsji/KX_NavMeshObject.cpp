...
static int buildRawVertIndicesData(DerivedMesh *dm, int *nverts_r, float **verts_r,
                            ...
                            int **recastData)
{
    ...
    trisToFacesMap = (int *)MEM_callocN(sizeof(int) * ntris, "buildRawVertIndicesData trisToFacesMap");
    tris = (unsigned short *)MEM_callocN(sizeof(unsigned short) * 3 * ntris, "buildRawVertIndicesData tris");
    tri = tris;
    triIdx = 0;
    for (fi = 0; fi < nfaces; fi++) {
        MFace *face = &faces[fi];
...
static int buildPolygonsByDetailedMeshes(const int vertsPerPoly, const int npolys,
                                  ...
                                  const int *dtrisToPolysMap)
{
    ...
    unsigned short *newPoly = (unsigned short *)MEM_callocN(sizeof(unsigned short) * capacity, "buildPolygonsByDetailedMeshes newPoly");
    ...
    for (polyidx = 0; polyidx < npolys; polyidx++) {
        ...
        unsigned char *traversedTris = (unsigned char *)MEM_callocN(sizeof(unsigned char) * dtrisNum, "buildPolygonsByDetailedMeshes traversedTris");
        ...
        while (tri != btri || edge != bedge) {
            ...
            if (neighbortri == 0xffff || dtrisToPolysMap[neighbortri] != polyidx + 1) {
                if (nv == capacity) {
                    ...
                    newPolyBig = (unsigned short *)MEM_callocN(sizeof(unsigned short) * capacity, "buildPolygonsByDetailedMeshes newPolyBig");
        ...
        adjustedPoly = (unsigned short *)MEM_callocN(sizeof(unsigned short) * nv, "buildPolygonsByDetailedMeshes adjustedPoly");
        adjustedNv = 0;
        for (i = 0; i < nv; i++) {
...
static int buildNavMeshData(const int nverts, const float *verts,
...

{
    ...
    dtris = (unsigned short *)MEM_callocN(sizeof(unsigned short) * 3 * 2 * ndtris, "buildNavMeshData dtris");
    ...
    dmeshes = (unsigned short *)MEM_callocN(sizeof(unsigned short) * npolys * 4, "buildNavMeshData dmeshes");
    ...
    vertsPerPoly = 6;
    polys = (unsigned short *)MEM_callocN(sizeof(unsigned short) * npolys * vertsPerPoly * 2, "buildNavMeshData polys");
...
static int buildNavMeshDataByDerivedMesh(DerivedMesh *dm, int *vertsPerPoly,
...
static int polyFindVertex(const unsigned short *p, const int vertsPerPoly, unsigned short vertexIdx)
{
    int i, res = -1;
    for (i = 0; i < vertsPerPoly; i++) {
        if (p[i] == 0xffff)
