...
inline void vset(float v[2], float x, float y)
{
  v[0] = x;
  v[1] = y;
}
...
static int sweepCircleCircle(const MT_Vector2 &pos0,
                             const MT_Scalar r0,
                             const MT_Vector2 &v,
                             const MT_Vector2 &pos1,
                             const MT_Scalar r1,
                             float &tmin,
                             float &tmax)
{
...
static int sweepCircleSegment(const MT_Vector2 &pos0,
                              const MT_Scalar r0,
                              const MT_Vector2 &v,
                              const MT_Vector2 &pa,
                              const MT_Vector2 &pb,
...
KX_ObstacleSimulation::KX_ObstacleSimulation(MT_Scalar levelHeight, bool enableVisualization)
    : m_levelHeight(levelHeight), m_enableVisualization(enableVisualization)
{
}
...
void KX_ObstacleSimulation::UpdateObstacles()
{
  for (size_t i = 0; i < m_obstacles.size(); i++) {
    if (m_obstacles[i]->m_type == KX_OBSTACLE_NAV_MESH ||
        m_obstacles[i]->m_shape == KX_OBSTACLE_SEGMENT)
      continue;
...
void KX_ObstacleSimulation::DrawObstacles()
{
  ...
  for (size_t i = 0; i < m_obstacles.size(); i++) {
    if (m_obstacles[i]->m_shape == KX_OBSTACLE_SEGMENT) {
      ...
      KX_RasterizerDrawDebugCircle(
          m_obstacles[i]->m_pos, m_obstacles[i]->m_rad, bluecolor, normal, SECTORS_NUM);
    }
  }
}
                                                      ...
                                                      MT_Scalar maxDeltaAngle)
{
  ...
  int obstidx = std::find(m_obstacles.begin(), m_obstacles.end(), activeObst) -
                m_obstacles.begin();
  if (obstidx == nobs)
    return;
  ...
  for (int iter = 0; iter < m_maxSamples; ++iter) {
    ...
    for (int i = 0; i < nobs; ++i) {
      ...
      if (ob->m_shape == KX_OBSTACLE_CIRCLE) {
        ...
        if (!sweepCircleCircle(activeObst->m_pos.to2d(),
                               activeObst->m_rad,
                               vab,
                               ob->m_pos.to2d(),
                               ob->m_rad,
        ...
        if (!sweepCircleSegment(activeObst->m_pos.to2d(),
                                activeObst->m_rad,
                                svel,
                                p1.to2d(),
                                p2.to2d(),
  ...
  for (int n = 0; n < nspos; ++n) {
    ...
    for (int i = 0; i < obstacles.size(); ++i) {
      ...
      if (ob->m_shape == KX_OBSTACLE_CIRCLE) {
        ...
        side += clamp(std::min(dot_v2v2(dp, vab), dot_v2v2(np, vab)) * 2.0f, 0.0f, 1.0f);
        nside++;
        ...
        if (dist_squared_to_line_segment_v2(activeObstPos, p, q) < sqr(r + ob->m_rad)) {
          ...
          if (!sweepCircleSegment(MT_Vector2(activeObstPos),
                                  r,
                                  MT_Vector2(vcand),
                                  MT_Vector2(p),
                                  MT_Vector2(q),
                                               ...
                                               const float maxDeltaAngle)
{
  ...
  if (!m_adaptive) {
    ...
    for (int k = 0; k < 5; ++k) {
      ...
      for (int y = 0; y < rad; ++y) {
        for (int x = 0; x < rad; ++x) {
          const float v_xy[2] = {res[0] + x * cs - half, res[1] + y * cs - half};
