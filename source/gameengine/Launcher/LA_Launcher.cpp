...
bool LA_Launcher::EngineNextFrame()
{
	...
	if (m_kxsystem && !m_exitRequested) {
		...
		if (m_inputDevice->GetEvent((SCA_IInputDevice::SCA_EnumInputs)m_ketsjiEngine->GetExitKey()).Find(SCA_InputEvent::ACTIVE)) {
		...
		else if (m_inputDevice->GetEvent(SCA_IInputDevice::WINCLOSE).Find(SCA_InputEvent::ACTIVE) ||
			m_inputDevice->GetEvent(SCA_IInputDevice::WINQUIT).Find(SCA_InputEvent::ACTIVE))
		{
