...
CValue *CBaseListValue::FindValue(const std::string &name) const
{
  const VectorTypeConstIterator it = std::find_if(
      m_pValueArray.begin(), m_pValueArray.end(), [&name](CValue *item) {
...
PyObject *CBaseListValue::buffer_concat(PyObject *self, PyObject *other)
{
  ...
  if (PyList_Check(other)) {
    ...
    for (Py_ssize_t i = 0; i < numitems; i++) {
      CValue *listitemval = listval->ConvertPythonToValue(
          PyList_GET_ITEM(other, i), true, "cList + pyList: CBaseListValue, ");
...
PyTypeObject CBaseListValue::Type = {PyVarObject_HEAD_INIT(nullptr, 0) "CListValue", /*tp_name*/
...
PyObject *CBaseListValue::Pyfilter(PyObject *args)
{
  ...
  try {
    ...
    PyErr_Format(
        PyExc_ValueError, "CList.filter(name, prop): invalid expression: %s.", error.what());
    return nullptr;
  }
