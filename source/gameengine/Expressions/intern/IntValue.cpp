...
CIntValue::CIntValue(cInt innie) : m_int(innie)
{
}

CIntValue::CIntValue(cInt innie, const std::string &name) : m_int(innie)
{
  SetName(name);
}
...
CValue *CIntValue::CalcFinal(VALUE_DATA_TYPE dtype, VALUE_OPERATOR op, CValue *val)
{
  ...
  switch (dtype) {
    ...
    case VALUE_BOOL_TYPE: {
      ret = new CErrorValue("[operator not valid on boolean and integer]" + op2str(op) +
                            GetText());
      break;
    }
