...
PyTypeObject PyObjectPlus::Type = {
    PyVarObject_HEAD_INIT(nullptr, 0) "PyObjectPlus", /*tp_name*/
...
PyObject *PyObjectPlus::py_base_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
  ...
  PyObjectPlus_Proxy *ret = (PyObjectPlus_Proxy *)type->tp_alloc(
  ...
  if (ret->py_ref) {
    ...
    Py_DECREF(base);
    if (ret->ref) {
      ret->ref->m_proxy = (PyObject *)
          ret;         // No need to add a ref because one is added when creating.
...
PyObject *PyObjectPlus::NewProxyPlus_Ext(PyObjectPlus *self,
                                         PyTypeObject *tp,
                                         void *ptr,
                                         bool py_owns)
{
...
void PyObjectPlus::ShowDeprecationWarning_func(const std::string &old_way,
