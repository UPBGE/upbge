...
SCA_ConstraintActuator::SCA_ConstraintActuator(SCA_IObject *gameobj,
                                               ...
                                               int option,
                                               char *property)
    : SCA_IActuator(gameobj, KX_ACT_CONSTRAINT), m_refDirVector(refDir), m_currentTime(0)
{
    ...
    switch (m_locrot) {
      ...
      case KX_ACT_CONSTRAINT_DIRNZ:
        ...
        }
        {
          ...
          KX_RayCast::Callback<SCA_ConstraintActuator, void> callback(
              this, dynamic_cast<PHY_IPhysicsController *>(spc));
          ...
          if (result) {
            ...
            if (m_option & KX_ACT_CONSTRAINT_DISTANCE) {
              if (m_posDampTime) {
                newdistance = filter * (position - callback.m_hitPoint).length() +
                              (1.0f - filter) * m_minimumBound;
              }
              else {
      ...
      case KX_ACT_CONSTRAINT_FHNZ:
        ...
        normal.normalize();
        {
          ...
          if (result) {
            ...
            MT_Vector3 newVelocity = spc->GetLinearVelocity() -
                                     (springForce + springDamp) * direction;
            if (m_option & KX_ACT_CONSTRAINT_NORMAL) {
              newVelocity += (springForce + springDamp) *
                             (newnormal - newnormal.dot(direction) * direction);
            }
            ...
            if (m_option & KX_ACT_CONSTRAINT_DOROTFH) {
              ...
              MT_Vector3 angDamp = angVelocity * ((m_refDirVector[1] > MT_EPSILON) ?
                                                      m_refDirVector[1] :
                                                      m_refDirVector[0]);
...
void SCA_ConstraintActuator::Clamp(MT_Scalar &var, float min, float max)
{
  if (var < min) {
    var = min;
  }
...
PyAttributeDef SCA_ConstraintActuator::Attributes[] = {
    ...
    EXP_PYATTRIBUTE_FLOAT_RW(
        "distance", -FLT_MAX, FLT_MAX, SCA_ConstraintActuator, m_minimumBound),
                                                   ...
                                                   const struct EXP_PYATTRIBUTE_DEF *attrdef)
{
  ...
  if (MT_fuzzyZero(len)) {
    PyErr_SetString(PyExc_ValueError,
                    "actuator.direction = vec: SCA_ConstraintActuator, invalid direction");
    return 1;
  }
