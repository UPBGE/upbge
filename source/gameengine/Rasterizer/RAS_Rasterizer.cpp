...
void RAS_Rasterizer::InitScreenShaders()
{
  ...
  DRW_shgroup_create(GPU_shader_get_builtin_shader(GPU_SHADER_DRAW_FRAME_BUFFER), nullptr);
  ...
  {
    DRWShadingGroup *shgrp =
  DRW_shgroup_create(GPU_shader_get_builtin_shader(GPU_SHADER_STEREO_ANAGLYPH), nullptr);
  ...
  {
    DRWShadingGroup *shgrp =
  DRW_shgroup_create(GPU_shader_get_builtin_shader(GPU_SHADER_STEREO_STIPPLE), nullptr);
    DRW_shgroup_uniform_int(shgrp, "lefteyetex", &zero, 1);
    DRW_shgroup_uniform_int(shgrp, "righteyetex", &one, 1);
...
void RAS_Rasterizer::DrawStereoFrameBuffer(RAS_ICanvas *canvas,
    ...
    case RAS_STEREO_ABOVEBELOW: {
      switch (eye) {
        case RAS_STEREO_LEFTEYE: {
          ...
          area.SetLeft(0);
          area.SetBottom(canvas->GetHeight() - int(canvas->GetHeight() - m_noOfScanlines) / 2);
...
MT_Matrix4x4 RAS_Rasterizer::GetFrustumMatrix(StereoEye eye,
                                              float left,
                                              float right,
                                              float bottom,
                                              float top,
...
MT_Matrix4x4 RAS_Rasterizer::GetViewMatrix(StereoEye eye,
                                           const MT_Transform &camtrans,
                                           bool perspective)
{
  // correction for stereo
  if (Stereo() && perspective) {
    static const MT_Vector3 unitViewDir(
        0.0f, -1.0f, 0.0f);  // minus y direction, Blender convention
    static const MT_Vector3 unitViewupVec(0.0f, 0.0f, 1.0f);

    const MT_Matrix3x3 &camOrientMat3x3 = camtrans.getBasis().transposed();
    // actual viewDir
    const MT_Vector3 viewDir =
        camOrientMat3x3 * unitViewDir;  // this is the moto convention, vector on right hand side
    // actual viewup vec
