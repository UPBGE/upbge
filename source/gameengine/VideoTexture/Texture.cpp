	...
	if (m_orgSaved)
	{
		...
		if (m_useMatTexture) {
			...
			if (m_imgTexture) {
				...
				GPU_texture_set_opengl_bindcode(m_imgTexture->gputexture[TEXTARGET_TEXTURE_2D], m_orgImg);
			}
		}
		else
		{
			...
			if (m_source != nullptr)
			{
				...
				if (!m_orgSaved)
				{
					...
					if (m_useMatTexture) {
						...
						if (m_imgTexture) {
							m_orgImg = GPU_texture_opengl_bindcode(m_imgTexture->gputexture[TEXTARGET_TEXTURE_2D]);
							GPU_texture_set_opengl_bindcode(m_imgTexture->gputexture[TEXTARGET_TEXTURE_2D], m_actTex);
						}
					}
					else
					{
